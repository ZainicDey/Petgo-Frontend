...
export default function ThreadCard({ thread, index = 0, showLine = false }: ThreadCardProps) {
  ...
  return (
    <article
      className="py-4 px-5 border-b border-white/20 flex gap-3 animate-fade-in-up last:border-b-0"
    ...
    >
      ...
      <div className="flex flex-col items-center gap-0 shrink-0">
        <div className="relative flex">
          {thread.avatar ? (
            <img src={thread.avatar} alt={thread.author} className="w-10 h-10 rounded-full object-cover block" />
          ) : (
            <div
        ...
        {showLine && <div className="w-[2px] flex-1 min-h-[24px] bg-white/10 rounded-sm mt-1.5" />}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-baseline gap-2 mb-1">
          ...
          <button className="bg-transparent border-none ml-auto text-white/35 cursor-pointer py-0.5 px-1.5 rounded-md text-lg leading-none transition-colors hover:bg-white/10 hover:text-white/70 font-inherit">···</button>
        </div>

        {/* Content */}
        <p className="text-base font-extralight leading-relaxed text-white/95 mb-2.5 break-words tracking-wide">{thread.content}</p>

        {/* Optional image */}
        {thread.image && (
          <img src={thread.image} alt="Thread media" className="w-full rounded-xl max-h-[380px] object-cover mb-2.5 block" />
        )}
